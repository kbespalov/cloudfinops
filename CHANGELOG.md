...
Дата среза каталога — 25 сентября 2026. Сверены публичные прайсы Yandex Cloud (Billing API), VK Cloud, Cloud.ru, T1 Cloud, MWS и Selectel.

- **Yandex Cloud:** в каталоге появились платформы Broadwell, Ice Lake Compute-Optimized, AMD Zen 4 и Zen 4 Compute-Optimized (vCPU и память, включая прерываемые). Прерываемая T4i — 21,09 ₽/час, обычная T4i по-прежнему 158,11 ₽/час. DeepSeek v4.1 Flash, Speech Realtime Max TTS Live и Speech Realtime DeepSeek V4 Flash — 300 ₽ и 500 ₽ за 1 млн входных и выходных токенов.
- **VK Cloud:** в каталоге появились высокочастотные процессоры, диски High-IOPS и High-IOPS HA, отдельные ставки vCPU мастера Kubernetes. Сами ставки прайса от 12 января 2026 не изменились.
- **Cloud.ru:** CDN по приложению 7.CRS.1, суммы с НДС 22%: до 100 ТБ — 1,098 ₽/ГБ, дальше — 0,854 ₽/ГБ, покрытие APAC — 6,10 ₽/ГБ. Foundation Models, тариф 7.EVO.11.2 от 15 сентября 2026: GigaChat Ultra — 96,2214 ₽ и 288,6032 ₽, GigaChat 3 Pro — 73,0292 ₽ и 176,3876 ₽ за 1 млн входных и выходных токенов с НДС.
- **T1 Cloud:** приложение от 7 сентября 2026. В каталоге появились прерываемые vCPU и память, зашифрованные диски, плата за запросы Object Storage и ставки vCPU и памяти GPU-серверов g1–g7. В прайсе суммы без НДС; для сравнения в каталоге они пересчитаны с НДС 22%.
- **MWS:** часовые ставки в каталоге приведены к четырём знакам после запятой, как в прайсе (NBS-PL2 — 0,0124 ₽, образы и снимки — 0,0048 ₽, мастер Kubernetes — 8,7855 ₽ и 23,0708 ₽). Добавлен GLM 5.3: 178,12 ₽ и 746,64 ₽ за 1 млн токенов. Публичная цена A100 216,0252 ₽/час начинает действовать 12 октября 2026 и записана как будущая.
- **Selectel:** облачные ставки vCPU, памяти, GPU, дисков, S3, IP, трафика, CDN и Kubernetes совпали с каталогом. Выделенный сервер HGX B300 (GL8-B300-HGX-25GE) в прайсе стоит 9 000 000 ₽/мес; в прошлой версии каталога было 8 000 000 ₽. Добавлены RTX 4090 на 48 ГБ и A4000. Дата проверки — 25 сентября 2026.

### Чат: график по просьбе

Если в вопросе есть просьба построить график, диаграмму или визуализацию, ответ в чате калькулятора и на странице чата может включать график. Столбчатый график строится по таблице с ценами в ₽. Другой вид модель задаёт блоком `chart`. Без такой просьбы график не показывается, в том числе если в ответе случайно оказался этот блок. «График работы» и явный отказ («без графика») графиком не считаются.

### Документация API: типографика и формулировки

- Раздел `/api` переведён на короткую шкалу Gravity UI: меньше кеглей, без микротекста 9–11 px и без декоративных надзаголовков.
- Описания методов, параметров и ответов переписаны как справочник: сначала контракт, затем ограничение и код ошибки. Один SKU отделён от расчёта конфигурации, `null` — от нуля, HTTP 200 — от статуса `quotes[].status`.
...
В публичном прайсе MWS появился третий класс S3 — **Cold**: хранение **1,26 ₽/ГиБ·мес**. Таксономия как у Yandex / VK / Cloud.ru / Selectel: capacity + **PUT/GET** (write/read family; в прайсе те же ставки на POST/PATCH/LIST и HEAD/OPTIONS — 0,0106 ₽ / 10 и 0,01 ₽ / 100). Источники: [тарификация Object Storage](https://mws.ru/docs/cloud-platform/storage/general/pricing.html) и [all-prices](https://mws.ru/docs/cloud-platform/about/general/mws-cloud-platform-pricing.html). Egress тот же, **1,3212 ₽/ГиБ**.
...
Добавили три tool'а в baseline chat: **`compare_inference_tco`** (Hosted API vs self-host + break-even), **`suggest_savings`** (рычаги экономии по ВМ/GPU), **`market_radar`** (срез рынка по корзинам). В пустом чате — три новых примера-вопроса; для stream — русские progress-лейблы.

### Каталог / Cloud.ru: Kimi K2.6 (local)

Добавили **kimi-k2.6** input/output в Evolution Foundation Models Cloud.ru (внутренняя модель): 175,68 / 725,90 ₽ за 1M с НДС по тарифу 7.EVO.11.2. Теперь локальный Kimi K2.6 сравнивается у MWS и Cloud.ru.

### Каталог / сравнение при бесплатном тарифе

Когда лучший exact-peer = 0 ₽: у платных бейдж **есть бесплатный** (не «есть 0 ₽»), у самого бесплатного бейдж не дублируем; баннер — «лучший — бесплатно».
...
### Каталог / find-similar: exact vs functional peers (P0)

«Найти похожие» разделён на широкий retrieval и eligibility-слой (`peer-match`): точные аналоги (hard dimensions) vs функциональные. Ценовые бейджи — только для `exact + priceEligible`; synthetic/derived и неверифицированные units не дают price claims. **0 ₽ остаётся в сравнении** (`equal-free` / `есть 0 ₽`), иначе якорем становится near-zero вроде Selectel 0,01 и появляются +7000%; при разбросе ≥500% показываем ×N, не проценты. Seed не подменяется. Для K8s retrieval — вся топология (Selectel/VK Small видны при Medium), exact — тот же size tier.

### Каталог / Yandex K8s: убрали паритет 2/6

У Yandex больше нет синтетики **2 vCPU / 6 ГиБ** (зональной и HA): в консоли такой пресет не заказать, сравнение Small идёт через заказной **2/8** и VK 2/6 · 2/4 · 2/8. Формы 2/4 у Yandex по-прежнему нет.

### E-ассистент / gpt-oss 70/30 → hosted API, не self-host
...
У Yandex в каталог добавили заказные пресеты мастера (API/консоль — только preset, не свободные vCPU/RAM): **4/8** (`c-c4-m8`) · **4/16** (`s-c4-m16`) · **8/16** (`c-c8-m16`) · **8/32** (`s-c8-m32`) · **16/32** (`c-c16-m32`) + HA 3×. Минимум по-прежнему **2/8**; форм **2/4** и **2/6** нет. CPU-optimized формы совпадают с Cloud.ru 4/8 · 8/16 · 16/32 для like-for-like.

### Каталог / «Найти похожие» для AI: exact modelId

«Найти похожие» по токенам больше не смешивает модели одной семьи: сравниваем exact `modelId` (gpt-oss-120b ≠ 20b, GLM-5.2 ≠ 4.7) + input/output. Грубая семья (chip) остаётся только для обычного browse.

### Каталог / Cloud.ru Managed Kubernetes masters

В каталог добавили все нативные зональные мастера из тарифного PDF Cloud.ru: **2/4 · 4/8 · 8/16 · 16/32** (и синтетику HA = 3× каждой формы). Строки «Managed Kubernetes ВМ …» (в т.ч. 64 vCPU / 256 GB) — это flavors воркеров, не control plane; в каталог мастеров их не тащим. Fractional vCPU (10%/30%) пока не добавляли.

### Каталог / Managed Kubernetes masters (Yandex · VK)

Убрали у Yandex синтетику **2 vCPU / 4 ГиБ** (зональную и HA): такой хост в консоли не создать, минимум мастера — **s-c2-m8 (2 / 8 ГиБ)**. Оставили **2/8** как заказной ориентир. У VK — формы 2/6 · 2/8 и паритет 2/4 с Cloud.ru; notes у синтетик явно говорят «оценка / не строка прайса», зачем форма и что workers не входят. `parityOnly` не участвует в дефолтном «дешёвом мастере», но доступен в find-similar по size tier.

### Каталог / «Найти похожие»: дельта цены и like-for-like

В режиме похожих в колонке цены — минимум зелёным числом, остальные с жёлтым «+N%» относительно минимума в отфильтрованном наборе. Диски сравниваем отдельно по ёмкости и по IOPS (NVMe GiB ≠ IOPS add-on). У K8s find-similar учитывает форму мастера (vCPU/RAM).
