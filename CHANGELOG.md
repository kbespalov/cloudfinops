...
У Yandex в каталог добавили заказные пресеты мастера (API/консоль — только preset, не свободные vCPU/RAM): **4/8** (`c-c4-m8`) · **4/16** (`s-c4-m16`) · **8/16** (`c-c8-m16`) · **8/32** (`s-c8-m32`) · **16/32** (`c-c16-m32`) + HA 3×. Минимум по-прежнему **2/8**; паритет **2/6** с VK; **2/4** нет. CPU-optimized формы совпадают с Cloud.ru 4/8 · 8/16 · 16/32 для like-for-like.
...
Убрали у Yandex синтетику **2 vCPU / 4 ГиБ** (зональную и HA): такой хост в консоли не создать, минимум мастера — **s-c2-m8 (2 / 8 ГиБ)**. Оставили **2/8** как заказной ориентир и **2/6** (`parityOnly`) — только чтобы примерно сравнивать с типичным стартом VK (STD2-2-6). У VK — зеркальные формы 2/6 · 2/8 и паритет 2/4 с Cloud.ru; notes у синтетик явно говорят «оценка / не строка прайса», зачем форма и что workers не входят. `parityOnly` не участвует в дефолтном «дешёвом мастере», но доступен в find-similar по shape.
