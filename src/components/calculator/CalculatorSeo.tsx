...
export function VmCalculatorSeo({
  ...
  return (
    <section className={styles.seo} aria-labelledby="calculator-vm-seo-title">
      ...
      <h3 className={styles.subtitle}>Калькуляторы стоимости по облакам</h3>
      <ul className={styles.list}>
        {CALCULATOR_PROVIDER_SEO.map((p) => (
          <li key={p.slug}>
            <a href={`/calculator/${p.slug}`}>
              <strong>Калькулятор стоимости {p.brandDisplay}</strong>
            </a>
            {' — '}
            {p.brandRu}
...
const VM_FAQ = [
  ...
  },
  {
    ...
    answer:
      'B300 в каталоге — выделенный узел Selectel GL8-B300-HGX-25GE (8×GPU), не обычная облачная GPU-ВМ. В калькуляторе: 8×B300, 128 CPU (2×Xeon 6767P), 2048 GiB RAM и bundle-цена dedicated по публичному прайсу.',
  },
  {
    ...
    '@graph': [
      {
        ...
        featureList: [
          ...
          'Калькулятор стоимости Yandex Cloud',
          'Калькулятор стоимости VK Cloud',
          'Калькулятор стоимости Selectel',
          'Калькулятор стоимости Cloud.ru',
          'Калькулятор стоимости MWS Cloud',
          'Калькулятор стоимости T1 Cloud',
