...
const CAPABILITIES: {title: string; text: string}[] = [
  ...
  {
    title: 'Расчёт GPU под инференс',
    text: 'Спросите, сколько H100/H200 нужно для Qwen, GLM или Kimi self-host: ассистент оценит VRAM и квант, сравнит цены узлов в облаках РФ и покажет hosted API (₽/1M input и output) как альтернативу аренде GPU.',
  },
];
...
const FAQ: {question: string; answer: string}[] = [
  ...
  },
  {
    ...
    answer:
      'Рекомендательная система приводит запрошенную конфигурацию ВМ или GPU к паритету по всем провайдерам, считает итоговую стоимость по публичным тарифам и показывает минимальную цену в каталоге Cloud FinOps, поясняя различия конфигураций.',
  },
  {
  ...
  {
    question: 'Как рассчитать GPU для инференса Qwen или GLM?',
    answer:
      'Задайте вопрос вроде «какая инфраструктура нужна, чтобы развернуть Qwen3-Coder-Next / GLM 5.2 self-host в РФ». Рекомендатель вернёт лестницу конфигов (INT4 PoC → FP8 на H200 → 2×H100 production), цены узлов и отдельно input/output API, без подмены соседними моделями.',
  },
  {
    question: 'Чем расчёт под инференс отличается от цены за токены?',
    answer:
      'Цена за 1M токенов — это hosted API. Расчёт под инференс — оценка self-host / dedicated GPU (VRAM, число карт, квант) и ₽/мес за узел. Ассистент умеет оба сценария и не смешивает метрики в одной колонке.',
  },
  {
    ...
    '@graph': [
      {
        ...
        inLanguage: 'ru-RU',
        description:
          'ИИ-ассистент FinOps по ценам облаков России: подбор ВМ и GPU, расчёт конфигурации под инференс LLM (Qwen, GLM, Kimi), сравнение H100/H200/A100 и AI API у Yandex Cloud, VK Cloud, Selectel, Cloud.ru, MWS и T1 по публичным тарифам.',
        featureList: [
