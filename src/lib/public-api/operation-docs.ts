...
export const OPERATION_DOCS: Record<RestOperationId, {title: string; description: string; section: string}> = {
  ...
  list_attributes: {
    title: 'Discover filterable attributes by category', section: 'attributes',
    description: 'Discover Product.attributes fields, value types, supported operators, units and closed enums (allowedValues). values lists observed values with per-category SKU counts, before other filters; it is not a closed enum for string fields. knownCount and missingCount describe coverage. Filter this directory by categories. eq is exact and case-sensitive; in is OR; numeric range includes min/max boundaries. Different attributes combine with AND and null never matches. Numbers describe existing SKU metadata, not arbitrary configurable resources. Arbitrary providerAttributes are not supported.',
  },
  list_services: {
...
For exact GPU models, CPU/RAM sizes and other characteristics, use attributes filters with eq, in or inclusive range; GET /attributes lists fields per category and observed values. The search_products input schema describes their types and categories. Null never matches. Numeric SKU metadata is not a request to assemble a configuration.
