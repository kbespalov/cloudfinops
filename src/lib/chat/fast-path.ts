...
- График — только если пользователь явно попросил (график / диаграмма / визуализация); без просьбы \`\`\`chart не выводи. По просьбе: столбцы по таблице с ₽ рисует интерфейс; иной вид — fenced \`\`\`chart JSON {kind:bar|line|area|pie, title, xField, categories, series}; числа только из tool results; не дублируй столбцы таблицы.`;
...
const ALIAS_PLANS: {id: string; match: RegExp; tools: FastPathTool[]}[] = [
  ...
  },
  {
    ...
    // «минимальная конфигурация …» → shape limits (intent 2c), not cheapest ₽.
    // Economy lattice: «минимальная ВМ / виртуальная машина», «самая дешёвая…».
    match:
      /(?:экономичн[а-яё]*\s+вариант|самая?\s+деш[её]в[а-яё]*|самый\s+деш[её]в[а-яё]*|минимальн[а-яё]*\s+(?:вм\b|виртуал)).{0,64}(?:кажд[а-яё]*.{0,16}провайдер|у\s+всех|по\s+провайдер)|(?:у\s+каждого|кажд[а-яё]*.{0,16}провайдер|по\s+провайдер).{0,64}(?:экономичн|самая?\s+деш[её]в|самый\s+деш[её]в|минимальн[а-яё]*\s+(?:вм\b|виртуал))|(?:подбери|покажи|дай).{0,24}самое?\s+экономичн/i,
    tools: [
      {
        name: 'get_quote',
...
export function looksGpuFullServerFollowUp(text: string): boolean {
  return /(?:сервер\w*\s+целиком|целиком\s+(?:сервер|нод)|не\s+просто\s+карт|с\s+хостом|gpu\s*\+\s*хост|полноценн\w*\s+(?:gpu|сервер|нод)|конфигураци\w*\s+целиком|паритет\w*\s+(?:по\s+)?конфигурац|таблиц\w*\s+с\s+паритет|пример\s+конфигурац|собери\s+(?:пример\s+)?конфигурац|если\s+собирать)/i.test(
    text,
  );
}
