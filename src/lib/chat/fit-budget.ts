...
export function fitBudget(options: {
  ...
  return {
    ...
    valuePick,
    note:
      'Ответь сразу таблицей по highlights (или 2–4 сценария из scenarios.best): Провайдер | Конфиг ×N | Итого ₽/мес | Утилизация % | Остаток ₽. Победитель — максимальная utilPct (плотная укладка бюджета), НЕ минимальный spend: дешевле pack = больше leftover. Не используй колонку «к минимуму» для fit_budget. Если valuePick задан — отдельной строкой: тот же объём ресурсов дешевле unit (часто ниже utilPct, больше leftover). Не пиши «провайдера нет», если он есть в scenarios/valuePick. Не устраивай опрос. count=число целых машин; utilPct=доля бюджета. Формы только из ответа инструмента.',
  };
}
