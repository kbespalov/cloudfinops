...
export function buildSkuComparePrompt(meter: CatalogMeter, period: PeriodMode): string {
  ...
  if (meter.categoryKey === 'gpu') {
    lines.push(
      'Найди ближайшие аналоги у других провайдеров и сравни цены в одной таблице. Аналог = тот же класс datacenter GPU и сопоставимое число карт (поколение/VRAM/HGX·NVLink), не любой NVIDIA и не consumer/entry (GTX/RTX 20xx/T4/L4) для B300/H200/H100. Нет того же поколения — ближайшее (например 8×H200/H100 к 8×B300) с явными отличиями; если близкого SKU нет — скажи прямо, не подставляй дешёвый мусор.',
    );
  } else {
    lines.push(
