...
function buildProviderSeo(input: {
  ...
  return {
    ...
    intro: `На этой странице можно рассчитать стоимость облачных ресурсов ${brandDisplay} и сравнить результат с предложениями других провайдеров. Индивидуальные скидки, специальные договорные условия и промоакции не учитываются.`,
    independenceNote: 'Независимый расчёт по открытым тарифам',
    independenceTooltip:
      `Cloud FinOps не связан с ${brandDisplay}. Расчёт выполнен по опубликованным тарифам и может отличаться от итогового коммерческого предложения.`,
    faq: [
      {
        question: `Откуда берутся тарифы ${brandDisplay} на Cloud FinOps?`,
        answer: `Мы используем только опубликованные тарифы ${brandDisplay} из открытых источников провайдера. Закрытые прайсы, партнёрские скидки и промоакции в расчёт не входят. Перед заказом сверьте актуальные условия на сайте ${brandDisplay}.`,
      },
      {
        question: `Что считает этот калькулятор?`,
        answer: `Калькулятор оценивает стоимость VM и GPU по опубликованным тарифам ${brandDisplay} и показывает цены других провайдеров для сопоставимой конфигурации.`,
      },
      {
        question: `Как посчитать стоимость ВМ?`,
        answer: `Выберите пресет или параметры (например 4 vCPU / 16 GiB). Калькулятор соберёт цену по тарифам ${brandDisplay} и покажет сравнение с рынком. Расчётный месяц = 720 часов. Для каждого тарифа используется налоговый статус из каталога и источника цены.`,
      },
      {
        question: `Это официальный калькулятор ${brandDisplay}?`,
        answer: `Нет. Cloud FinOps — независимый информационно-аналитический сервис и не связан с ${brandDisplay}. Мы не заменяем официальный прайс и калькулятор провайдера. Перед заказом проверьте итоговую стоимость и доступность ресурсов на сайте ${brandDisplay}.`,
      },
      ...(input.faqExtra ?? []),
    ],
  };
