...
export const newsItems: NewsItem[] = [
  // ——— Cloud FinOps (hub → blog / tools) ———
  {
    ...
    summary:
      'Открыли программный доступ к каталогу SKU, публичным тарифам российских облаков и расчёту стоимости конфигураций compute и GPU. Для приложений доступен REST API, а для AI-клиентов — MCP-сервер; подключение не требует API-ключа.',
    body: [
      'Через API можно искать продукты по характеристикам, фильтровать их по провайдеру, категории и региону, изучать правила тарификации и находить альтернативы. Калькулятор подбирает конфигурации по минимально необходимым ресурсам и возвращает стоимость каждого компонента.',
      'В первой версии расчёт выполняется за месяц из 720 часов по публичным тарифам каталога. Наличие мощностей API не проверяет, а при нехватке тарифных данных возвращает причину неполного расчёта. В статье рассказываем о возможностях REST и MCP, приводим первый запрос и ссылки на документацию.',
    ],
    tags: ['finops', 'ai'],
    sourceUrl: 'https://cloudfinops.ru/blog/cloudfinops-public-api',
  ...
  },
  {
    ...
    summary:
      'MWS Cloud добавила в Object Storage новый класс Cold для данных, которые нужно хранить долго, но читать редко: бэкапов, архивов, сырых логов и датасетов после обучения моделей.',
    body: [
      'Хранение в Cold стоит 1,26 ₽/ГиБ в месяц с НДС — почти вдвое дешевле Standard. При этом операции чтения и записи стоят дороже, поэтому класс выгоднее всего для больших объёмов данных с редкими обращениями.',
      'Теперь в Object Storage MWS Cloud доступны три класса: Warm на NVMe для высоконагруженных сценариев и ML, Standard для повседневных нагрузок и Cold для архивного хранения.',
      'Класс можно выбрать на уровне бакета или отдельного объекта при загрузке. В Cloud FinOps новый тариф уже добавлен в каталог для расчёта стоимости хранения и операций.',
    ],
    tags: ['storage', 'finops'],
