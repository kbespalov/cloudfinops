...
export const INFERENCE_MODELS: InferenceModelProfile[] = [
  ...
  },
  {
    ...
    parameterCountNote:
      'Официальный релиз V4-Flash-0731 (31.07.2026): та же архитектура 284B / 13B active + модуль DSpark (на HF суммарно ~304B), контекст до 1M. Память считают все эксперты, не только active. Родной FP4+FP8 ≈158–167 ГиБ; MIT.',
    weights: [
      {
        dtype: 'int4',
        weightFormat: 'nvfp4',
        checkpointSizeGiB: 167,
        weightsVramGiB: 167,
        theoreticalLowerBoundGiB: 142,
        effectiveBitsPerWeight: 4.5,
        quantizedComponents: 'MoE experts (FP4); most other params FP8; + DSpark draft',
        compatibleRuntimes: ['vLLM', 'SGLang'],
        supportedGpuArch: ['Hopper', 'Blackwell'],
        source:
          'deepseek-ai/DeepSeek-V4-Flash-0731 — FP4+FP8 Mixed ~158 GB weights; ~167 GB с DSpark',
        confidence: 'measured',
      },
      {
        dtype: 'int8',
        ...
        weightsVramGiB: 170,
        theoreticalLowerBoundGiB: 142,
        source: 'Community INT8 / W8A8 serving estimate for 284B MoE (+ DSpark headroom)',
        confidence: 'estimated',
      },
      {
        dtype: 'fp8',
    ...
    recommended: [
      {
        ...
        quant: 'int4',
        interconnect: 'NVLink',
        estimatedVramGiB: 230,
        notes:
          'Официальный FP4+FP8 Instruct (0731 / DSpark). Не считайте «284B × 1 байт» — смотрите размер чекпойнта; с KV на 1M бюджет ≈170–180 ГиБ.',
      },
      {
        gpuFamily: 'H100',
        gpuCount: 4,
        ...
        notes:
          'Рабочий FP8: Flash-Base / FP8 Mixed (~284 ГиБ) на 4×H100. Официальный vLLM-пример 0731 — 4×GPU + expert parallel + DSpark.',
      },
      {
        gpuFamily: 'A100',
        gpuCount: 4,
        quant: 'int4',
        interconnect: 'NVLink',
        estimatedVramGiB: 250,
        notes:
          'На 4×A100 (320 ГиБ) — INT4/FP4+FP8; 2×A100 (160 ГиБ) ниже пола ~170 ГиБ с KV. Полный BF16 / толстый FP8 не влезут.',
      },
    ],
    ...
    sources: [
      ...
      'DeepSeek-V4-Flash-0731 (MIT) — official 284B/13B + DSpark, 1M context; post-training agentic upgrade 2026-07-31',
    ],
    checkedAt: '2026-08-02',
    caveats: [
      'В память идут все 284B параметров (+ draft DSpark); 13B active — только про вычисления на токен.',
      'Две H200 (~282 ГиБ) хватит для INT4/FP4+FP8. Для FP8 берите от четырёх H100/H200 и конкретный чекпойнт.',
      'Для reasoning effort max держите окно ≥384K — KV растёт заметно.',
